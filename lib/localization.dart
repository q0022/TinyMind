class AppTranslations {
  static const Map<String, Map<String, String>> _localizedValues = {
    'th': {
      'app_title': 'TinyMind',
      'dashboard': 'Dashboard',
      'settings': 'การตั้งค่า',
      'dictionary': 'คำละเว้น',
      'today': 'วันนี้',
      'lifetime': 'ทั้งหมด',
      
      // Sidebar
      'accessibility_status': 'Accessibility',
      'accessibility_granted': 'อนุญาตแล้ว',
      'accessibility_needed': 'ต้องการสิทธิ์',
      
      // Dashboard Tab
      'performance_title': 'ประสิทธิภาพการทํางาน',
      'performance_subtitle': 'สถิติและประวัติการแก้คำผิดแบบเรียลไทม์',
      'stat_words_corrected': 'แก้ไขคำผิดแล้ว',
      'stat_layout_fixed': 'สลับภาษา Layout',
      'stat_ai_requests': 'เรียกประมวลผล AI',
      'stat_saved_chars': 'ประหยัดตัวอักษรพิมพ์',
      'stat_hotkey_used': 'สลับด้วย Hotkey',
      'recent_activity': 'ประวัติการทำงานล่าสุด (Recent Activity)',
      'empty_history': 'ยังไม่มีประวัติการแก้ไขคำผิดในรอบนี้ค่ะ พิมพ์เพื่อทดสอบได้เลยนะ!',
      'sandbox_title': 'Keyboard Sandbox (ทดสอบลบเกิน / ลบขาด)',
      'sandbox_reset': 'รีเซ็ต Sandbox',
      'sandbox_desc': 'พื้นที่จำลองสำหรับพิมพ์ทดสอบ: ระบบจะทำการเว้นวรรค 15 ช่องด้านหน้าไว้ เพื่อตรวจจับว่ามีการลบเกินพิกัดล้ำเข้ามาในส่วนของช่องว่างหรือไม่',
      
      // Settings Tab
      'settings_title': 'การตั้งค่าระบบ',
      'settings_subtitle': 'ปรับแต่งพฤติกรรมและการทำงานของ TinyMind',
      'accessibility_warning_title': 'ต้องขอสิทธิ์ Accessibility สำหรับดักจับปุ่มกดภายนอกค่ะ',
      'accessibility_warning_desc': 'การปิดแอปพลิเคชันอื่นและเขียนข้อความใหม่จำเป็นต้องใช้สิทธิ์นี้ รันสิทธิ์ในระบบแล้วเปิดการอนุญาตให้แอป TinyMind ด้วยนะจ๊ะ',
      'accessibility_btn': 'เปิดหน้าการอนุญาตของระบบ',
      
      'general_group': 'การทํางานทั่วไป',
      'autocorrect_enable': 'เปิดใช้งานระบบ Autocorrection',
      'autocorrect_enable_desc': 'เปิดเพื่อตรวจจับปุ่มพิมพ์ผิดและสลับภาษาอัตโนมัติในแอปอื่นๆ',
      'launch_at_login': 'เปิดพร้อมกับการบูตระบบ (Launch at Login)',
      'launch_at_login_desc': 'เปิดแอปพลิเคชันและรันเบื้องหลังทันทีที่เปิดเครื่องคอมพิวเตอร์',
      
      'appearance_group': 'ปรับแต่งหน้าตา (Appearance)',
      'dark_mode': 'โหมดกลางคืน (Dark Mode)',
      'dark_mode_desc': 'เปิดการใช้งานโหมดมืด (ปิดหากต้องการใช้ธีมสว่างสดใส)',
      'primary_color': 'โทนสีหลักของแอป (Primary Color)',
      'display_language': 'ภาษาที่ใช้แสดง (Display Language)',
      
      'hotkey_group': 'ปุ่มลัดสลับภาษา (Hotkey Settings)',
      'custom_hotkey_enable': 'กำหนดปุ่มลัดด้วยตัวเอง (Use Custom Hotkey)',
      'custom_hotkey_enable_desc': 'เปิดเพื่อเลือกปุ่มลัดสลับภาษาเอง (หากปิด จะใช้ปุ่มร่วมเดิมทั้ง 3 แบบ)',
      'modifier_label': 'ปุ่มร่วม (Modifier)',
      'key_label': 'ปุ่มหลัก (Key)',
      'none_modifier': 'None (ไม่ใช้ปุ่มร่วม)',
      
      'engine_group': 'เครื่องมือประมวลผลคำแปล (Correction Engine)',
      'correction_mode_title': 'โหมดเครื่องมือตรวจแก้',
      'correction_mode_desc': 'เลือกว่าจะใช้กฎอักษร (Regex) หรือปัญญาประดิษฐ์ (AI) ในการตรวจและแปลงภาษา',
      'mode_regex': 'ใช้เฉพาะกฎและพจนานุกรม (Regex)',
      'mode_ai': 'ใช้ปัญญาประดิษฐ์เป็นหลัก (AI)',
      'mode_hybrid': 'ไฮบริด (พจนานุกรมก่อน + AI สำรอง)',
      'fast_local_engine': 'ระบบแก้คำและ Layout สลับภาษาในตัว (Fast Local Engine)',
      'fast_local_engine_desc': 'วิเคราะห์ Layout ผิดพลาด (ไทย <-> อังกฤษ) ทันทีหลังกด Spacebar/Enter ด้วยความเร็วระดับมิลลิวินาที',
      'local_ai_engine': 'ระบบวิเคราะห์บริบทด้วย Local AI (Alibaba Qwen)',
      'local_ai_engine_desc': 'รันโมเดลภาษาขนาดเล็ก Qwen (GGUF) ภายในเครื่องเพื่อตรวจแก้ข้อความตามบริบทด้วย llama.cpp',
      'gguf_model_label': 'ไฟล์โมเดล GGUF ในเครื่อง',
      'gguf_model_loading_auto': 'ยังไม่ได้เลือกโมเดล (กรุณาเปิดใช้งานเพื่อโหลดอัตโนมัติ)',
      'gguf_model_downloading': 'กำลังคัดลอก/ดาวน์โหลดโมเดล...',
      'gguf_model_pick_btn': 'เลือกไฟล์ GGUF',
      'gguf_model_loading_vram': 'กำลังโหลดโมเดลเข้า VRAM/RAM... (โปรดรอสักครู่)',
      'gguf_model_loaded_success': 'โมเดลโหลดเข้า VRAM เรียบร้อยแล้ว พร้อมทำงาน!',
      'auto_switch_length_title': 'สลับเลย์เอาต์ภาษาอัตโนมัติตามความยาวตัวอักษร',
      'auto_switch_length_desc': 'สลับคีย์บอร์ดและแปลงคำทันทีเมื่อพิมพ์คำศัพท์ในเลย์เอาต์อังกฤษแต่น่าจะเป็นภาษาไทยถึงเกณฑ์ โดยไม่ต้องรอเว้นวรรค',
      'auto_switch_threshold': 'เกณฑ์ความยาวตัวอักษรสำหรับสลับอัตโนมัติ',
      'chars_suffix': 'ตัวอักษร',
      'auto_switch_hint': 'ช่วงที่แนะนำคือ 8 ตัวอักษร (หากสั้นเกินไปอาจทำให้สลับผิดพลาดสำหรับคำศัพท์ภาษาอังกฤษทั่วไป)',
      'enable_slash_commands': 'เปิดใช้งานคำสั่งสแลช (Slash Commands)',
      'enable_slash_commands_desc': 'อนุญาตให้ใช้งานคำสั่งที่ขึ้นต้นด้วย / เช่น /t หรือ /translate เพื่อแปลภาษารวดเร็ว',
      'settings_code_filter_title': 'คัดกรองโค้ดและสัญลักษณ์ (Code & Symbol Filter)',
      'settings_code_filter_desc': 'เปิดเพื่อป้องกันไม่ให้สลับภาษาเมื่อป้อนอักขระพิเศษสำหรับเขียนโค้ด (ปิดสวิตช์นี้หากต้องการให้สัญลักษณ์และทุกข้อความผ่านตรงไปประมวลผลทันที)',
      'keyboard_layouts_group': 'คีย์บอร์ดต่างประเทศ (CJK Layouts)',
      'auto_detect_os_keyboards': 'ตรวจจับคีย์บอร์ดของระบบอัตโนมัติ',
      'auto_detect_os_keyboards_desc': 'ตรวจหาคีย์บอร์ดที่เพิ่มไว้ในระบบ macOS อัตโนมัติ เพื่อเปิด/ปิดการสลับภาษาให้สอดคล้องกัน',
      'korean_layout': 'แป้นพิมพ์เกาหลี (Korean Layout)',
      'japanese_layout': 'แป้นพิมพ์ญี่ปุ่น (Japanese Layout)',
      'chinese_layout': 'แป้นพิมพ์จีน (Chinese Layout)',
      'keyboard_detected': 'เปิดใช้งาน (ตรวจพบแป้นพิมพ์ในเครื่อง)',
      'keyboard_not_detected': 'ปิดใช้งาน (ไม่พบแป้นพิมพ์ในเครื่อง)',
      'enable_korean': 'เปิดใช้งานเลย์เอาต์เกาหลี (Korean Layout)',
      'enable_korean_desc': 'สลับภาษาและแปลงคำสำหรับแป้นพิมพ์เกาหลี',
      'enable_japanese': 'เปิดใช้งานเลย์เอาต์ญี่ปุ่น (Japanese Layout)',
      'enable_japanese_desc': 'สลับภาษาและแปลงคำสำหรับแป้นพิมพ์ญี่ปุ่น',
      'enable_chinese': 'เปิดใช้งานเลย์เอาต์จีน (Chinese Layout)',
      'enable_chinese_desc': 'สลับภาษาและแปลงคำสำหรับแป้นพิมพ์จีน',
      
      // Dictionary Tab
      'dict_title': 'การจัดการคำและคีย์ลัด',
      'dict_subtitle': 'จัดการรายการคำละเว้นจากการสลับแป้นพิมพ์ หรือคำย่อส่วนตัวสำหรับอำนวยความสะดวกในการพิมพ์',
      'dict_tab_ignore': 'คำละเว้น (Ignore List)',
      'dict_tab_shortcuts': 'คีย์ลัดคำย่อ (Text Shortcuts)',
      'dict_input_hint': 'พิมพ์คำละเว้น เช่น api, admin, db, เจน, boy',
      'dict_add_btn': 'เพิ่มคำศัพท์',
      'dict_empty': 'ยังไม่มีคำศัพท์ละเว้นในขณะนี้ค่ะ',
      'dict_clear_all_btn': 'ล้างพจนานุกรมส่วนตัวทั้งหมด',
      'ignored_count_label': 'จำนวนคำศัพท์ที่ละเว้น',
      'clear_dict_confirm_title': 'ยืนยันการล้างพจนานุกรมส่วนตัว',
      'clear_dict_confirm_desc': 'คุณต้องการล้างรายการคำศัพท์ที่ละเว้น และประวัติการเรียนรู้คำภาษาอังกฤษทั้งหมดใช่หรือไม่? การกระทำนี้ไม่สามารถย้อนกลับได้ค่ะ',
      'clear_dict_success': 'ล้างพจนานุกรมส่วนตัวทั้งหมดเรียบร้อยแล้วค่ะ',
      'cancel': 'ยกเลิก',
      'confirm': 'ยืนยัน',
      'shortcut_key_label': 'คำย่อ (เช่น tm)',
      'shortcut_val_label': 'คำเต็ม (เช่น TinyMind)',
      'shortcut_key_hint': 'คำย่อ',
      'shortcut_val_hint': 'คำเต็มสำหรับการขยายความ',
      'shortcut_add_btn': 'เพิ่มคีย์ลัด',
      'shortcut_empty': 'ยังไม่มีคีย์ลัดคำย่อในขณะนี้ค่ะ',
      
      // Dialogs
      'model_prepare_failed': 'เตรียมโมเดลล้มเหลว: ',
      'model_pick_failed': 'เกิดข้อผิดพลาดในการเลือกไฟล์: ',
      'upgrade_to_qwen_btn': '✨ อัปเกรดเป็นโมเดล Qwen 1.5B (แนะนำ - ฉลาดและเร็วมาก)',
      'model_connecting': 'กำลังเชื่อมต่อเซิร์ฟเวอร์เพื่อดาวน์โหลดโมเดล AI (1.04 GB)...',
      'model_copying_dev': 'ตรวจพบไฟล์โมเดลในโปรเจกต์ กำลังคัดลอกไฟล์เข้าเครื่อง...',
      'model_copying_progress': 'กำลังคัดลอกโมเดล...',
      'model_downloading_progress': 'กำลังดาวน์โหลดโมเดลภาษา...',
      'Layout (Auto)': 'สลับภาษาอัตโนมัติ',
      'Layout': 'สลับภาษา',
      'Local AI': 'วิเคราะห์ด้วย AI',
      'Th Layout': 'สลับเป็นไทย',
      'En Layout': 'สลับเป็นอังกฤษ',
      'Shortcut': 'ทางลัดข้อความ',
      
      // App Updates
      'update_available': 'มีอัปเดตเวอร์ชันใหม่!',
      'update_download_btn': 'ดาวน์โหลดตัวอัปเดต',
      'updates_section': 'การอัปเดตระบบ',
      'current_version_label': 'เวอร์ชันปัจจุบัน:',
      'check_updates_desc': 'ตรวจสอบและติดตั้งแอป TinyMind เวอร์ชันล่าสุดโดยอัตโนมัติ',
      'check_updates_btn': 'ตรวจสอบเวอร์ชันใหม่',
      'checking_updates_status': 'กำลังตรวจสอบ...',
      'no_updates_status': 'แอปเป็นเวอร์ชันล่าสุดแล้ว',

      // Diagnostics
      'diagnostics_section': 'การวินิจฉัยและบันทึกประวัติ',
      'export_logs_title': 'ส่งออกไฟล์ประวัติการทำงาน (Logs)',
      'export_logs_desc': 'เปิดโฟลเดอร์เก็บข้อมูลประวัติของแอป สำหรับส่งให้นักพัฒนานำไปวิเคราะห์แก้ไขปัญหา',
      'export_logs_btn': 'เปิดโฟลเดอร์ Log',

      // Tray Menu
      'tray_open': 'เปิดหน้าต่างหลัก',
      'tray_pause': 'ปิดการทำงานชั่วคราว',
      'tray_resume': 'เปิดการทำงาน',
      'tray_quit': 'ออกจากแอป',
    },
    'en': {
      'app_title': 'TinyMind',
      'dashboard': 'Dashboard',
      'settings': 'Settings',
      'dictionary': 'Ignore List',
      'today': 'Today',
      'lifetime': 'Lifetime',
      
      // Sidebar
      'accessibility_status': 'Accessibility',
      'accessibility_granted': 'Granted',
      'accessibility_needed': 'Requires Permission',
      
      // Dashboard Tab
      'performance_title': 'Work Efficiency',
      'performance_subtitle': 'Real-time statistics and correction history',
      'stat_words_corrected': 'Words Corrected',
      'stat_layout_fixed': 'Layouts Switched',
      'stat_ai_requests': 'AI Inferences',
      'stat_saved_chars': 'Saved Keystrokes',
      'stat_hotkey_used': 'Hotkey Used',
      'recent_activity': 'Recent Activity',
      'empty_history': 'No correction history yet. Type something to test!',
      'sandbox_title': 'Keyboard Sandbox (Over/Under-deletion Test)',
      'sandbox_reset': 'Reset Sandbox',
      'sandbox_desc': 'Simulation area for typing tests: 15 spaces are padded in front to detect if backspaces delete into the empty spacing padding.',
      
      // Settings Tab
      'settings_title': 'System Settings',
      'settings_subtitle': 'Adjust behavior and settings of TinyMind',
      'accessibility_warning_title': 'Accessibility permission required to hook global keystrokes',
      'accessibility_warning_desc': 'Closing other applications and typing replacements requires this permission. Please enable TinyMind in System Settings.',
      'accessibility_btn': 'Open System Preferences',
      
      'general_group': 'General Settings',
      'autocorrect_enable': 'Enable Autocorrection',
      'autocorrect_enable_desc': 'Detect keyboard layout errors and automatically switch language in other apps',
      'launch_at_login': 'Launch at Login',
      'launch_at_login_desc': 'Open application and run in background on startup',
      
      'appearance_group': 'Appearance Settings',
      'dark_mode': 'Dark Mode',
      'dark_mode_desc': 'Enable dark theme (disable for vibrant light theme)',
      'primary_color': 'Primary Theme Color',
      'display_language': 'Display Language',
      
      'hotkey_group': 'Hotkey Settings',
      'custom_hotkey_enable': 'Use Custom Hotkey',
      'custom_hotkey_enable_desc': 'Enable to select custom hotkeys; if disabled, default hotkeys are used',
      'modifier_label': 'Modifier Key',
      'key_label': 'Main Key',
      'none_modifier': 'None (No Modifier)',
      
      'engine_group': 'Correction Engine',
      'correction_mode_title': 'Correction Strategy',
      'correction_mode_desc': 'Choose the primary engine for layout switching and spelling correction.',
      'mode_regex': 'Rule-based (Regex Only)',
      'mode_ai': 'AI-based (Alibaba Qwen)',
      'mode_hybrid': 'Hybrid (Regex first + AI fallback)',
      'fast_local_engine': 'Fast Local Layout Switcher',
      'fast_local_engine_desc': 'Correct layout typos (Thai <-> English) instantly after Spacebar/Enter in milliseconds',
      'local_ai_engine': 'Contextual Local AI (Alibaba Qwen)',
      'local_ai_engine_desc': 'Run local Qwen model (GGUF) to correct contextually via llama.cpp',
      'gguf_model_label': 'Local GGUF Model Path',
      'gguf_model_loading_auto': 'Model not selected; enable AI to download automatically',
      'gguf_model_downloading': 'Copying/downloading model file...',
      'gguf_model_pick_btn': 'Select GGUF File',
      'gguf_model_loading_vram': 'Loading model into VRAM/RAM... (Please wait)',
      'gguf_model_loaded_success': 'Model successfully loaded into VRAM. Ready!',
      'auto_switch_length_title': 'Auto Switch Layout by Word Length',
      'auto_switch_length_desc': 'Switch layout immediately when typed characters reach threshold before pressing Space',
      'auto_switch_threshold': 'Length Threshold for Auto Switch',
      'chars_suffix': 'characters',
      'auto_switch_hint': '8 characters is recommended to prevent false switches on English words',
      'enable_slash_commands': 'Enable Slash Commands',
      'enable_slash_commands_desc': 'Allow executing quick actions starting with / (e.g., /t or /translate)',
      'settings_code_filter_title': 'Code & Symbol Filter',
      'settings_code_filter_desc': 'Filter layout switches on programming code symbols (turn off to allow all characters/symbols to switch layout)',
      'keyboard_layouts_group': 'Multilingual Keyboard Layouts',
      'auto_detect_os_keyboards': 'Auto-detect macOS keyboards',
      'auto_detect_os_keyboards_desc': 'Automatically detect enabled keyboard layouts in macOS and active CJK switches accordingly',
      'korean_layout': 'Korean Layout (Hangul)',
      'japanese_layout': 'Japanese Layout (Kana)',
      'chinese_layout': 'Chinese Layout (Pinyin)',
      'keyboard_detected': 'Active (Detected in macOS)',
      'keyboard_not_detected': 'Inactive (Not detected in macOS)',
      'enable_korean': 'Enable Korean Layout',
      'enable_korean_desc': 'Autocorrect typos on Korean layout',
      'enable_japanese': 'Enable Japanese Layout',
      'enable_japanese_desc': 'Autocorrect typos on Japanese layout',
      'enable_chinese': 'Enable Chinese Layout',
      'enable_chinese_desc': 'Autocorrect typos on Chinese layout',
      
      // Dictionary Tab
      'dict_title': 'Words & Shortcuts',
      'dict_subtitle': 'Manage words excluded from layout switching, or custom text abbreviations',
      'dict_tab_ignore': 'Ignore List',
      'dict_tab_shortcuts': 'Text Shortcuts',
      'dict_input_hint': 'Type ignored word (e.g. api, admin, db, boy)',
      'dict_add_btn': 'Add Word',
      'dict_empty': 'No ignored words yet.',
      'dict_clear_all_btn': 'Clear All Personal Dictionary',
      'ignored_count_label': 'Ignored Words Count',
      'clear_dict_confirm_title': 'Confirm Clearing Personal Dictionary',
      'clear_dict_confirm_desc': 'Do you want to clear all ignored words and learned English words history? This action cannot be undone.',
      'clear_dict_success': 'Successfully cleared all personal dictionary entries.',
      'cancel': 'Cancel',
      'confirm': 'Confirm',
      'shortcut_key_label': 'Abbreviation (e.g. tm)',
      'shortcut_val_label': 'Expansion (e.g. TinyMind)',
      'shortcut_key_hint': 'Abbrev',
      'shortcut_val_hint': 'Expansion text',
      'shortcut_add_btn': 'Add Shortcut',
      'shortcut_empty': 'No custom shortcuts yet.',
      
      // Dialogs
      'model_prepare_failed': 'Model preparation failed: ',
      'model_pick_failed': 'Error selecting file: ',
      'upgrade_to_qwen_btn': '✨ Upgrade to Qwen 1.5B (Recommended - Fast & Smart)',
      'model_connecting': 'Connecting to server to download AI model (1.04 GB)...',
      'model_copying_dev': 'Dev model file detected. Copying file to application support directory...',
      'model_copying_progress': 'Copying model...',
      'model_downloading_progress': 'Downloading language model...',
      'Layout (Auto)': 'Layout (Auto)',
      'Layout': 'Layout',
      'Local AI': 'Local AI',
      'Th Layout': 'Thai Layout',
      'En Layout': 'English Layout',
      'Shortcut': 'Shortcut',
      
      // App Updates
      'update_available': 'New version available!',
      'update_download_btn': 'Download update',
      'updates_section': 'System Updates',
      'current_version_label': 'Current Version:',
      'check_updates_desc': 'Check for and install the latest version of TinyMind automatically',
      'check_updates_btn': 'Check for Updates',
      'checking_updates_status': 'Checking...',
      'no_updates_status': 'App is up to date',

      // Diagnostics
      'diagnostics_section': 'Diagnostics & Logs',
      'export_logs_title': 'Export Application Logs',
      'export_logs_desc': 'Open the folder containing application logs to send to the developer for debugging',
      'export_logs_btn': 'Open Logs Folder',

      // Tray Menu
      'tray_open': 'Open Window',
      'tray_pause': 'Pause',
      'tray_resume': 'Resume',
      'tray_quit': 'Quit',
    }
  };

  static String translate(String key, String locale) {
    final Map<String, String>? translations = _localizedValues[locale];
    if (translations == null) return key;
    return translations[key] ?? key;
  }
}
